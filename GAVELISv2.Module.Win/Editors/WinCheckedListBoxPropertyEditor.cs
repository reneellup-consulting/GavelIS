...
namespace GAVELISv2.Module.Win.Editors
{
    ...
    public class WinCheckedListBoxPropertyEditor : WinPropertyEditor, IComplexPropertyEditor
    {
        ...
        protected override void ReadValueCore()
        {
            ...
            if (PropertyValue is XPBaseCollection)
            {
                ...
                try
                {
                    ...
                    {
                        Control.SetItemChecked(dataSource.IndexOf((obj as TireInsCondDetail).Condition), true);
                    }
                }
                catch (Exception)
                {
        ...
        void control_ItemCheck(object sender, DevExpress.XtraEditors.Controls.ItemCheckEventArgs e) {
            ...
            Tire tr = checkedItems.Session.GetObjectByKey<Tire>((CurrentObject as Tire).Oid);
            TireInspectionCondition tic = obj as TireInspectionCondition;
            var data = tr.TireInspectionConditions.Where(o => o.Condition == tic);
            TireInsCondDetail ticd = data.LastOrDefault();
            if (ticd == null)
            {
                ticd = new TireInsCondDetail(checkedItems.Session);
                ticd.TireId = tr;
