...
{
    public partial class GenerateCargoParticularsUnitController : ViewController
    {
        private SimpleAction generateCargoParticularsAction;
        private TripsCargoHeader _TripsCargoHeader;
        private System.ComponentModel.BackgroundWorker _BgWorker;
        private ProgressForm _FrmProgress;
        public GenerateCargoParticularsUnitController()
        {
            this.TargetObjectType = typeof(TripsCargoHeader);
            this.TargetViewType = ViewType.DetailView;
            string actionID = "TripsCargoHeader.GenerateCargoParticularsUnit";
            this.generateCargoParticularsAction = new SimpleAction(this,
            actionID, PredefinedCategory.RecordEdit);
            this.generateCargoParticularsAction.Caption = "Generate (U)";
        ...
        void generateCargoParticularsAction_Execute(object sender, SimpleActionExecuteEventArgs e)
        {
            ...
            for (int i = _TripsCargoHeader.TripsCargoDetailsUnit.Count - 1; i >= 0; i--)
            {
                _TripsCargoHeader.TripsCargoDetailsUnit[i].Delete();
            }
            ...
            SelectedData result = session.ExecuteSproc("GetCargoParticularsUnit",
        ...
        private void BgWorkerDoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < rows.Count(); i++)
                {
                    ...
                    index++;
                    TripsCargoDetailUnit detail = null;
                    if (!units.ContainsKey(Guid.Parse(rows[i].Values[2].ToString())))
                    {
                        detail = ReflectionHelper.CreateObject<TripsCargoDetailUnit>(session);
                        detail.ParentID = tch;
                        // Driver
                        detail.Unit = session.GetObjectByKey<FATruck>(Guid.Parse(rows[i].Values[2].ToString()));
                        units.Add(Guid.Parse(rows[i].Values[2].ToString()), detail.Unit.No);
                    }
                    else
                    {
                        detail = tch.TripsCargoDetailsUnit.Where(o => o.Unit.Oid == Guid.Parse(rows[i].Values[2].ToString())).FirstOrDefault();
                    }
                    ...
                    {
                        case "PINEAPPLE":
                            regs = collection.Where(o => o.TruckNo == detail.Unit).Select(o => o).Where(o => o.Particular.Category.Category == "PINEAPPLE").Select(o => o);
                            // PineappleTrips
                            detail.PineappleTrips = Convert.ToInt32(rows[i].Values[5]);
                            // PineappleIncome
                            detail.PineappleIncome = regs.Select(o => o.NetBilling).Sum();
                            break;
                        case "BANANA":
                            regs = collection.Where(o => o.TruckNo == detail.Unit).Select(o => o).Where(o => o.Particular.Category.Category == "BANANA").Select(o => o);
                            // BananaTrips
                            detail.BananaTrips = Convert.ToInt32(rows[i].Values[5]);
                            // BananaIncome
                            detail.BananaIncome = regs.Select(o => o.NetBilling).Sum();
                            break;
                        case "CEMENT":
                            regs = collection.Where(o => o.TruckNo == detail.Unit).Select(o => o).Where(o => o.Particular.Category.Category == "CEMENT").Select(o => o);
                            // CementTrips
                            detail.CementTrips = Convert.ToInt32(rows[i].Values[5]);
                            // CementIncome
                            detail.CementIncome = regs.Select(o => o.NetBilling).Sum();
                            break;
                        case "OTHERS":
                            regs = collection.Where(o => o.TruckNo == detail.Unit).Select(o => o).Where(o => o.Particular.Category.Category == "OTHERS").Select(o => o);
                            // OtherTrips
                            detail.OtherTrips = Convert.ToInt32(rows[i].Values[5]);
                            // OtherIncome
