...
{
    public partial class UpdateBalanceCustomer : ViewController
    {
        //private Customer _Customer;
        private System.ComponentModel.BackgroundWorker _BgWorker;
        private ProgressForm _FrmProgress;

        public UpdateBalanceCustomer()
        {
            InitializeComponent();
            RegisterActions(components);
        }

        private void UpdateCustomerBalance_Execute(object sender, SimpleActionExecuteEventArgs e)
        {
            if (((DevExpress.ExpressApp.ListView)this.View).SelectedObjects.Count == 0)
            {
                XtraMessageBox.Show("There are no customer(s) selected",
            ...
            _FrmProgress = new ProgressForm("Updating customers...", count,
            "Updating customers {0} of {1} ");
        ...
        private void BgWorkerDoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                Customer cs = null;
                foreach (Customer item in trans)
                {
                    cs = session.GetObjectByKey<Customer>(item.Oid);
                    cs.UpdateBalance(true);
                    cs.Save();
...
ProgressChangedEventArgs e)
        ...
        RunWorkerCompletedEventArgs e)
        {
            ...
            {
                XtraMessageBox.Show(
                    "Updating customer operation has been cancelled", "Cancelled",
        ...
        private void UpdateActionState(bool inProgress)
        {
            this.UpdateCustomerBalance.
                Enabled.SetItemValue("Updating customer", !inProgress);
        }
