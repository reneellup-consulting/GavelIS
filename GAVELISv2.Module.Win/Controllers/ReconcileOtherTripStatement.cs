...
namespace GAVELISv2.Module.Win.Controllers {
    public partial class ReconcileOtherTripStatement : ViewController {
        ...
        SimpleActionExecuteEventArgs e) {
            ...
            _BgWorker.RunWorkerAsync(otherTripStatement);
        }
        private void ReconcileOtherTripStatement_Executed(object sender, 
        ActionBaseEventArgs e) {
            ...
            //ObjectSpace.CommitChanges();
            //ObjectSpace.ReloadObject(otherTripStatement);
            //ObjectSpace.Refresh();
        ...
        private void BgWorkerDoWork(object sender, DoWorkEventArgs e) {
            ...
            OtherTripStatement _otherTripStatement = (OtherTripStatement)e.
            Argument;
            OtherTripStatement thisStatement = session.GetObjectByKey<
            OtherTripStatement>(_otherTripStatement.Oid);
            try {
                #region Process Charge Adjustment
                foreach (OtherTripCharge item in thisStatement.OtherTripCharges) 
                {
                    if (item.Pay) {
                        #region --> Process Invoice
                        if (item.SourceType.Code == "OT") {
                            OtherTrip invoice = null;
                            ...
                            if (item.Discount == 0 && item.FinanceCharge == 0) {
                                if (item.Adjust == item.OpenAmount) {
                                    invoice = session.GetObjectByKey<OtherTrip>(
                                    item.SourceID);
                ...
                foreach (OtherTripPayment item in thisStatement.
                OtherTripPayments) {
                    if (item.Select) {
        ...
        private void UpdateActionState(bool inProgress) { 
            reconcileStanTripStatement.Enabled.SetItemValue(
