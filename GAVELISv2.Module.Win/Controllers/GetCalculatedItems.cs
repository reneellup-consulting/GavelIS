...
{
    public partial class GetCalculatedItems : ViewController
    {
        private SimpleAction getCalculatedItems;
        private StaffPayrollBatch _StaffPayrollBatch;
        private System.ComponentModel.BackgroundWorker _BgWorker;
        private ProgressForm _FrmProgress;

        public GetCalculatedItems()
        {
            this.TargetObjectType = typeof(StaffPayrollBatch);
            this.TargetViewType = ViewType.DetailView;
            string actionID = "StaffPayrollBatch.GetCalculatedItems";
            this.getCalculatedItems = new SimpleAction(this,
            actionID, PredefinedCategory.RecordEdit);
            this.getCalculatedItems.Caption = "Get Calculated Items";
            this.getCalculatedItems.Execute += new SimpleActionExecuteEventHandler(
GetCalculatedItems_Execute);

        }
        private void GetCalculatedItems_Execute(object sender,
...
CurrentObject as StaffPayrollBatch;
            ...
            try
            {
                for (int i = _StaffPayrollBatch.CalculatedRecords.Count - 1;
                i >= 0; i--)
                {
                    _StaffPayrollBatch.CalculatedRecords[i].PayrollBatchID = null;
                }
            }
            catch (Exception)
            {
...
"Employee records processed {0} of {1} ");
        ...
        private void BgWorkerDoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                ICollection calcRecords;
                DevExpress.Xpo.Metadata.XPClassInfo calcRecordsClass = thisIS.Session.GetClassInfo(typeof(
                CalculatedRecord));
                ...
                foreach (Employee emp in employees)
                {
                    ...
                    calcRecords = thisIS.Session.GetObjects(calcRecordsClass
                   , criteria1, sortProps1, 0, false, true);

                    foreach (CalculatedRecord crs in calcRecords)
                    {
                        crs.PayrollBatchID = thisIS;
                        crs.Save();
                    }
        ...
        RunWorkerCompletedEventArgs e)
        {
            ...
            {
                XtraMessageBox.Show(
                    "Extraction of calculated records is cancelled",
            ...
            else
            {
                ...
                {
                    XtraMessageBox.Show(
                    "Extraction of calculated records has been successfully generated."
                    );
