...
{
    public partial class ReleaseECSController : ViewController
    {
        private EmployeeChargeSlip chargeSlip;
        private SimpleAction releaseECSAction;
        private System.ComponentModel.BackgroundWorker _BgWorker;
        private ProgressForm _FrmProgress;
        public ReleaseECSController()
        {
            this.TargetObjectType = typeof(EmployeeChargeSlip);
            this.TargetViewType = ViewType.DetailView;
            string actionID = string.Format("{0}.ReleaseECS", this.GetType().
            Name);
            this.releaseECSAction = new SimpleAction(this, actionID,
            PredefinedCategory.RecordEdit);
            this.releaseECSAction.Caption = "Release ECS";
            this.releaseECSAction.Execute += new
            SimpleActionExecuteEventHandler(ReleaseECS_Execute);
            this.releaseECSAction.Executed += new EventHandler<
            ActionBaseEventArgs>(ReleaseECS_Executed);
            this.releaseECSAction.ConfirmationMessage =
            "Do you really want to release this Charge Slip?";
            UpdateActionState(false);
        }
        private void ReleaseECS_Execute(object sender,
        SimpleActionExecuteEventArgs e)
        {
            ...
            {
                throw new ApplicationException("Cannot release unapproved charge slip");
            }
            if (chargeSlip.Released)
            {
                throw new UserFriendlyException("The items of this Charge Slip was already released.");
            }
            //if (chargeSlip.EmployeeChargeSlipItemDetails.Count == 0)
            //{
            //    throw new UserFriendlyException("There are no items to release in this Charge Slip.");
            //}
            if (chargeSlip.CompanyInfoHead.AllowInsufficientCurrQty != true)
            {
                foreach (var item in chargeSlip.EmployeeChargeSlipItemDetails)
                {
        ...
        private void ReleaseECS_Executed(object sender,
        ...
        private void BgWorkerDoWork(object sender, DoWorkEventArgs e)
        {
            ...
            EmployeeChargeSlip args = (EmployeeChargeSlip)e.Argument;
            EmployeeChargeSlip thisEcs = session.GetObjectByKey<EmployeeChargeSlip>(args.Oid);
            ...
            try
            {
                ...
                foreach (EmployeeChargeSlipItemDetail ecsid in thisEcs.EmployeeChargeSlipItemDetails)
                {
                    ...
                    //_icj.OutQty = Math.Abs(dets.BaseQTY);
                    ...
                    //_icj.Cost = dets.ItemNo.Cost;
                    //_icj.Price = dets.Price;
                    //_icj.UOM = dets.BaseUOM;
                    ...
                    _message = string.Format("Releasing item details {0} succesfull.", ecsid.Oid);
                ...
                foreach (EmployeeChargeSlipExpenseDetail ecscd in thisEcs.EmployeeChargeSlipExpenseDetails)
                {
                    ...
                    ecscd.Save();
                    _message = string.Format("Releasing expense detail {0} succesfull.", ecscd.Oid);
                    _BgWorker.ReportProgress(1, _message);
                    index++;
                }
            ...
            {
                if (index == thisEcs.EmployeeChargeSlipItemDetails.Count + thisEcs.EmployeeChargeSlipExpenseDetails.Count)
                {
        ...
        RunWorkerCompletedEventArgs e)
        {
            ...
            {
                XtraMessageBox.Show(
                    "Releasing ECS items has been cancelled", "Cancelled",
        ...
        {
            releaseECSAction.
                Enabled.SetItemValue("Releasing items", !inProgress);
        }
