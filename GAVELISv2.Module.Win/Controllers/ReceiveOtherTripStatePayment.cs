...
{
    public partial class ReceiveOtherTripStatePayment : ViewController
    {
        private OtherTripStatement otherTripStatement;
        private SimpleAction receiveOtherTripStatePayment;
        private NewObjectViewController newController;
        private DetailView payBillDetailView;
        public ReceiveOtherTripStatePayment()
        {
            this.TargetObjectType = typeof(OtherTripStatement);
            this.TargetViewType = ViewType.DetailView;
            string actionID = string.Format("{0}.ReceivePayment", this.GetType()
            .Name);
            this.receiveOtherTripStatePayment = new SimpleAction(this, actionID, 
            PredefinedCategory.RecordEdit);
            this.receiveOtherTripStatePayment.Caption = "Receive Payment";
            this.receiveOtherTripStatePayment.Execute += new 
            SimpleActionExecuteEventHandler(ReceiveOtherTripStatePayment_Execute)
            ;
            //this.receiveStanTripStatePayment.Executed += new EventHandler<
            //ActionBaseEventArgs>(ReceiveStanTripStatePayment_Executed);
        }
        private void ReceiveOtherTripStatePayment_Execute(object sender, 
