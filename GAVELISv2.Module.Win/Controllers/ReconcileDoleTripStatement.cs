...
namespace GAVELISv2.Module.Win.Controllers {
    public partial class ReconcileDoleTripStatement : ViewController {
        private DolefilTripStatement doleTripStatement;
        private SimpleAction reconcileDoleTripStatement;
        private System.ComponentModel.BackgroundWorker _BgWorker;
        public ReconcileDoleTripStatement() {
            this.TargetObjectType = typeof(DolefilTripStatement);
            this.TargetViewType = ViewType.DetailView;
            string actionID = string.Format("{0}.ReconcileStatement", this.
            GetType().Name);
            this.reconcileDoleTripStatement = new SimpleAction(this, actionID, 
            PredefinedCategory.RecordEdit);
            this.reconcileDoleTripStatement.Caption = "Reconcile";
            this.reconcileDoleTripStatement.Execute += new 
            SimpleActionExecuteEventHandler(ReconcileDoleTripStatement_Execute);
            this.reconcileDoleTripStatement.Executed += new EventHandler<
            ActionBaseEventArgs>(ReconcileDoleTripStatement_Executed);
            this.reconcileDoleTripStatement.ConfirmationMessage = 
            "Do you really want to reconcile the selected entries?";
            UpdateActionState(false);
        }
        private void ReconcileDoleTripStatement_Execute(object sender, 
        SimpleActionExecuteEventArgs e) {
            doleTripStatement = ((DevExpress.ExpressApp.DetailView)this.View).
            CurrentObject as DolefilTripStatement;
            if (Math.Round(doleTripStatement.Balance, 2) != 0) {throw new 
            ...
            _BgWorker.RunWorkerAsync(doleTripStatement);
        }
        private void ReconcileDoleTripStatement_Executed(object sender, 
        ActionBaseEventArgs e) {
            ...
            //ObjectSpace.CommitChanges();
            //ObjectSpace.ReloadObject(doleTripStatement);
            //ObjectSpace.Refresh();
        ...
        private void BgWorkerDoWork(object sender, DoWorkEventArgs e) {
            ...
            DolefilTripStatement _doleTripStatement = (DolefilTripStatement)e.
            Argument;
            DolefilTripStatement thisStatement = session.GetObjectByKey<
            DolefilTripStatement>(_doleTripStatement.Oid);
            try {
                #region Process Charge Adjustment
                foreach (DolefilTripCharge item in thisStatement.
                DolefilTripCharges) {
                    if (item.Pay) {
                        #region --> Process Invoice
                        if (item.SourceType.Code == "DF") {
                            DolefilTrip invoice = null;
                            ...
                            if (item.Discount == 0 && item.FinanceCharge == 0) {
                                if (item.Adjust == item.OpenAmount) {
                                    invoice = session.GetObjectByKey<DolefilTrip
                                    >(item.SourceID);
                ...
                foreach (DolefilTripPayment item in thisStatement.
                DolefilTripPayments) {
                    if (item.Select) {
        ...
        private void UpdateActionState(bool inProgress) { 
            reconcileDoleTripStatement.Enabled.SetItemValue(
