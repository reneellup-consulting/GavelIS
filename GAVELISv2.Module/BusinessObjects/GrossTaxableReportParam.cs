...
{
    //[NonPersistent]
    public class GrossTaxableReportParam : ReportParametersObjectBase
    {
        public GrossTaxableReportParam(Session session) : base(session) { }
        public override CriteriaOperator GetCriteria()
        {
            ...
            sbCrit.AppendFormat("[ExactEntryDate] >= #{0}# And [ExactEntryDateEnd] <= #{1}#  And [Vendor.VatTaxable] = True And [InvoiceType] = 'Charge'"
                , startDate.ToString("yyyy-MM-dd"), endDate.ToString("yyyy-MM-dd"));
            //sbCrit.AppendFormat("[ExactEntryDate] >= #{0}# And [ExactEntryDateEnd] <= #{1}#  And [Vendor.VatTaxable] = True"
            //    , startDate.ToString("yyyy-MM-dd"), endDate.ToString("yyyy-MM-dd"));
