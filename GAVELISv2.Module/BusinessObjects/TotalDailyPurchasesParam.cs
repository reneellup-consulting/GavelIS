...
{
    //[NonPersistent]
    public class TotalDailyPurchasesParam : ReportParametersObjectBase
    {
        public TotalDailyPurchasesParam(Session session) : base(session) { }
        public override CriteriaOperator GetCriteria()
        {
            ...
            // Date Range
            sbCrit.AppendFormat("[Status] <> 'Disapproved' And [ExactEntryDate] >= #{0}# And [ExactEntryDateEnd] <= #{1}#", _Start.ToString("yyyy-MM-dd"), _End.ToString("yyyy-MM-dd"));
            if (_Vendor != null)
            {
                sbCrit.AppendFormat(" And [Vendor.Code] = '{0}'", _Vendor.No);
            }
