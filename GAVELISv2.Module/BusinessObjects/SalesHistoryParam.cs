...
{
    //[NonPersistent]
    public class SalesHistoryParam : ReportParametersObjectBase
    {
        public SalesHistoryParam(Session session) : base(session) { }
        public override CriteriaOperator GetCriteria()
        {
            ...
            // Date Range
            sbCrit.AppendFormat("[InvoiceInfo.ExactEntryDate] >= #{0}# And [InvoiceInfo.ExactEntryDateEnd] <= #{1}#", _Start.ToString("yyyy-MM-dd"), _End.ToString("yyyy-MM-dd"));
            if (!_AllType)
            {
                sbCrit.AppendFormat(" And [InvoiceInfo.InvoiceType] == '{0}'", _InvoiceType);
            }
