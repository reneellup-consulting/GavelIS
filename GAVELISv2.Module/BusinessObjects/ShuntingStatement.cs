...
namespace GAVELISv2.Module.BusinessObjects {
    ...
    [CreatableItem(false)]
    public class ShuntingStatement : XPObject {
        ...
        public void UpdateSelectedPayments(bool forceChangeEvent) {
            ...
            foreach (ShuntingPayment detail in ShuntingPayments) {tempTotal += 
        ...
        public void UpdateDiscountAllowed(bool forceChangeEvent) {
            ...
            decimal tempTotal = 0m;
            foreach (ShuntingCharge detail in ShuntingCharges) {tempTotal += 
        ...
        public void UpdateFinCharges(bool forceChangeEvent) {
            ...
            decimal tempTotal = 0m;
            foreach (ShuntingCharge detail in ShuntingCharges) {tempTotal += 
                detail.Pay ? detail.FinanceCharge : 0;}
        ...
        public void UpdateVatTotal(bool forceChangeEvent)
        {
            ...
            decimal tempTotal = 0m;
            foreach (ShuntingCharge detail in ShuntingCharges)
            {
                tempTotal +=
                    detail.ShuntingEntryId.VATAmount;
            }
            _VatTotal = tempTotal;
            if (forceChangeEvent)
            {
        ...
        [Persistent("ChassyRental")]
        private decimal? _ChassyRental;
        [PersistentAlias("_ChassyRental")]
        public decimal? ChassyRental
        {
            get {
                try
                {
                    if (!IsLoading && !IsSaving && _ChassyRental == null)
                    {
                        UpdateChassyRental(false);
                    }
                }
                catch (Exception)
                {
                }
                return _ChassyRental; }
        }
        public void UpdateChassyRental(bool forceChangeEvent)
        {
            decimal? oldChassyRental = _ChassyRental;
            decimal tempTotal = 0m;
            foreach (ShuntingCharge detail in ShuntingCharges)
            {
                tempTotal +=
                    detail.ShuntingEntryId.ChassyRental;
            }
            _ChassyRental = tempTotal;
            if (forceChangeEvent)
            {
                OnChanged("VatTotal", VatTotal,
                    _VatTotal);
        ...
        [PersistentAlias("_Insurance")]
        public decimal? Insurance
        {
            get {
                try
                {
                    if (!IsLoading && !IsSaving && _Insurance == null)
                    {
        ...
        public void UpdateInsurance(bool forceChangeEvent)
        {
            decimal? oldInsurance = _Insurance;
            decimal tempTotal = 0m;
            foreach (ShuntingCharge detail in ShuntingCharges)
            {
                tempTotal +=
                    detail.ShuntingEntryId.Insurance;
            }
            _Insurance = tempTotal;
            if (forceChangeEvent)
            {
                OnChanged("Insurance", Insurance,
                    _Insurance);
            }
            ;
        }
        ...
        public void UpdateTotal(bool forceChangeEvent)
        {
            ...
            {
                tempTotal +=
                    detail.ShuntingEntryId.Total1;
            }
            _Total = tempTotal;
            if (forceChangeEvent)
            {
                OnChanged("Total", Total,
                    _Total);
            }
            ;
        }
        ...
        public void UpdateGrossBilling(bool forceChangeEvent) {
            ...
            decimal tempTotal = 0m;
            foreach (ShuntingCharge detail in ShuntingCharges) {tempTotal += 
                detail.ShuntingEntryId.GrossBilling;}
        ...
        public void UpdateWHTTotal(bool forceChangeEvent) {
            ...
            decimal tempTotal = 0m;
            foreach (ShuntingCharge detail in ShuntingCharges) {tempTotal += 
                detail.ShuntingEntryId.WHTAmount;}
            _WHTTotal = tempTotal;
        ...
        public XPCollection<ShuntingCharge> ShuntingCharges { get { return 
                GetCollection<ShuntingCharge>("ShuntingCharges"); } }
        [Aggregated,
        Association("ShuntingStatement-ShuntingPayments")]
        public XPCollection<ShuntingPayment> ShuntingPayments { get { return 
                GetCollection<ShuntingPayment>("ShuntingPayments"); } }
        public ShuntingStatement(Session session): base(session) {
