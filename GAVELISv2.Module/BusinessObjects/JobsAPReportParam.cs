...
{
    public class JobsAPReportParam : ReportParametersObjectBase
    {
        public JobsAPReportParam(Session session) : base(session) { }
        public override CriteriaOperator GetCriteria()
        {
            ...
            {
                sbCrit.AppendFormat("Not [Status] In ('Paid') And ");
            }
            else
                sbCrit.AppendFormat("Not [Status] In ('Paid') And [ExactEntryDate] >= #{0}# And [ExactEntryDateEnd] <= #{1}# And ", _Start.ToString("yyyy-MM-dd"), _End.ToString("yyyy-MM-dd"));
