...
{
    //[NonPersistent]
    public class CurrentReceiptReportParam : ReportParametersObjectBase
    {
        public CurrentReceiptReportParam(Session session) : base(session) { }
        public override CriteriaOperator GetCriteria()
        {
            ...
            {
                sbCrit.AppendFormat("[Status] In ('Current') And ");
            } else
                sbCrit.AppendFormat("[Status] In ('Current') And [ExactEntryDate] >= #{0}# And [ExactEntryDateEnd] <= #{1}# And ", _Start.ToString("yyyy-MM-dd"), _End.ToString("yyyy-MM-dd"));
